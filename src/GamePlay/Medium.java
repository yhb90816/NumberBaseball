...
public class Medium extends JPanel {
	...
	public void BaseSetting() {
		...
		exit.setBounds(450, 683, 120, 50);
		add(b1);add(b2);add(b3);add(b4);add(b5);add(b6);add(b7);add(b8);add(b9);add(b0);add(d);add(l);add(exit);
		...
		b0.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					if(t1.getText().length()==8) {
						...
						JOptionPane.showMessageDialog(null, "4자리 숫자가 아닙니다.", "Error", JOptionPane.ERROR_MESSAGE);
						t1.setText("");
					}
			}
		});
